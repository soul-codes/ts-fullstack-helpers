...
export interface ShapeOptions<Optional extends boolean = false> {
  optional?: Optional;
}

export type ShapeSchemaValue<
  Shape extends SchemaShape,
  Optional extends boolean
> = inferObjectNativeType<Shape> | inferEmptyType<Optional>;
export type ShapeSchemaError<Shape extends SchemaShape> =
...
> extends BaseSchema<
  "object",
  ShapeSchemaValue<Shape, Optional>,
  ShapeSchemaError<Shape>,
  ShapeOptions<Optional>
> {
  constructor(readonly shape: Shape, options: ShapeOptions<Optional> = {}) {
    super(options);
  }
  ...
  > {
    ...
    for (const key in shape) {
      ...
      if (result.ok) {
        ...
        hasProblems = true;
        problems[key] = result.error as ShapeSchemaError<Shape>;
      }
    }
...
export function shape<Shape extends SchemaShape>(
  shape: Shape,
  options?: ShapeOptions<false>
): ShapeSchema<Shape, false>;
export function shape<Shape extends SchemaShape>(
  shape: Shape,
  options?: ShapeOptions<true>
): ShapeSchema<Shape, true>;
export function shape<Shape extends SchemaShape>(
  shape: Shape,
  options: ShapeOptions<boolean> = {}
): ShapeSchema<Shape, boolean> {
  return new ShapeSchema(shape, options);
}
